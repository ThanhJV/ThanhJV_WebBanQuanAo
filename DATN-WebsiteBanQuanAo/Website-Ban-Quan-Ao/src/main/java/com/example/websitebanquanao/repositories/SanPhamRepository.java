...
public interface SanPhamRepository extends JpaRepository<SanPham, UUID> {
    ...
    @Query("select new com.example.websitebanquanao.infrastructures.responses.SanPhamResponse(sp.id,sp.idLoai.id, sp.idThuongHieu.id, sp.idCauLacBo.id, sp.ten, sp.ngay_tao, sp.anh, sp.idLoai.ten,sp.idThuongHieu.ten,sp.idCauLacBo.ten,sp.trang_thai) from SanPham sp where sp.id = :id")
